...
The Rust core supports manual updates for packaged Windows, Linux AppImage, and macOS builds. Update controls are not yet exposed in the UI. Development builds can check and download releases, but cannot replace their installation.
...
`CancelAppUpdateRequest` cancels checks, downloads, and pending installation preparation. Once shutdown has committed the helper, cancellation is unavailable. Changing channels cancels outstanding work and invalidates the candidate. There are no startup checks or automatic downloads.
...
`cargo test -p app-update` exercises file transactions and launches the real helper against fixture application processes. Core tests cover release discovery, pinned downloads, checksums, cancellation, and channel changes. Flutter tests cover the exit handoff and active-task confirmation. Package CI runs helper tests on Linux, Windows, and macOS.
