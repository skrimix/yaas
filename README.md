# YAAS

YAAS is a cross-platform desktop application for sideloading applications and managing Meta Quest headsets. Built with a Material Design 3 UI (Flutter) and a Rust core.

## Downloads

[Windows](https://github.com/skrimix/yaas/releases/download/nightly/YAAS-windows-x64.zip) | [Linux](https://github.com/skrimix/yaas/releases/download/nightly/YAAS-linux-x86_64.AppImage) | [MacOS](https://github.com/skrimix/yaas/releases/download/nightly/YAAS-macos.zip)

## Screenshots

![](./screenshots/1_home.png)
![](./screenshots/2_app_management.png)
![](./screenshots/3_download_apps.png)
![](./screenshots/4_app_details.png)
![](./screenshots/5_tasks_list.png)
![](./screenshots/6_downloads_list.png)


## Portable Mode

On Linux and Windows, you can enable portable mode:

```bash
./yaas --portable
```

In portable mode, application data is stored alongside the app in `_portable_data`.


## Data Storage

YAAS uses the following data directories by default:

- macOS: `~/Library/Application Support/YAAS`
- Linux: `$XDG_DATA_HOME/YAAS` or `$HOME/.local/share/YAAS`
- Windows: `%APPDATA%\YAAS`

## License

This project is licensed under the MIT License. See `LICENSE` for details.

The screenshots use [Breeze icons](https://develop.kde.org/frameworks/breeze-icons/)
for window controls. The icons are copyright KDE and licensed under the GNU LGPL
version 3 or later.
