    ...
    if write_legacy_release {
        if let Some(app) = cached {
            ...
            if let Err(e) = async {
                ...
                let last_updated = convert_legacy_last_updated(&app.last_updated)?;
            ...
            .await
            {
                warn!(
                    ...
                    app_full_name, "Failed to write legacy release.json"
                );
            }
        } else {
        ...
        if let Some(dt) = meta.downloaded_at {
            ...
            if let Err(e) = tokio::fs::rename(&meta_path_alt, &new_path).await {
                warn!(
                    ...
                    "Failed to rename our release.json to metadata.json"
                );
            }
        }
    }
...
fn convert_legacy_last_updated(s: &str) -> Result<String> {
    ...
    let re = regex!(r"(\d{4}-\d{2}-\d{2}) (\d{2}:\d{2}) UTC");
